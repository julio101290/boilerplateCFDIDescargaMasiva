...
use julio101290\boilerplateCFDIDescargaMasiva\Models\{
...
class PeticionesdescargamasivaController extends BaseController {
    ...
    public function getPeticionesdescargamasiva() {
        ...
        /*
         * Verificamos si tiene archivo ZIP
         */
        ...
        if ($datosPeticionesdescargamasiva["nombreArchivo"] != "") {


            $zipfile = $datosPeticionesdescargamasiva["nombreArchivo"];
            ...
            $totalXMLEnZIP = $cfdiReader->count();

            if ($totalXMLEnZIP > 0) {


                $totalXMLGrabados = $this->XMLArchivo->select("id")
                        ->where("uuidPaquete", $datosPeticionesdescargamasiva["uuidPeticion"])
                        ->countAllResults();
            }
        }

        $datosPeticionesdescargamasiva["totalXMLEnZIP"] = $totalXMLEnZIP;
        $datosPeticionesdescargamasiva["totalXMLGrabados"] = $totalXMLGrabados;
    ...
    public function save() {
        ...
        if ($datos["status"] == "descargado") {
            ...
            foreach ($cfdiReader->cfdis() as $uuid => $content) {
                 ...
                 * 
                 */
                $UUIDTotales = $this->XMLArchivo->select("id")
                                ->where("uuidTimbre", $uuid)->countAllResults();
                    ...
                    foreach ($traslados as $key) {
                        ...
                        if ($key["TasaOCuota"] == 0.16) {
                            ...
                            if (is_numeric($key["Importe"] && is_numeric($key["totalImpuestos16"]))) {
                                ...
                                $datosXML["totalImpuestos16"] = number_format($datosXML["totalImpuestos16"], 6) + number_format($key["Importe"], 6);
                            }
                        }
